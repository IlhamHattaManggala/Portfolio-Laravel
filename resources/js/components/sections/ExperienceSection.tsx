    ...
    <section className="py-20 md:py-32 relative overflow-hidden" id="experience">
      <div className="max-w-5xl mx-auto px-6">
        <div className="text-center mb-24">
          ...
          >
            {t('experience.title_1')} <span className="text-gray-400 dark:text-gray-500">{t('experience.title_2')}</span>
          </motion.h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          {displayExperiences.map((exp, i) => (
            ...
            >
              ...
              <div className="relative h-full bg-white dark:bg-[#0A0A0A]/80 backdrop-blur-xl border border-black/10 dark:border-white/10 rounded-[2rem] p-6 sm:p-8 hover:border-primary/50 shadow-sm dark:shadow-none transition-all duration-300 flex flex-col">
                <div className="flex flex-col gap-6 flex-grow">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 relative shrink-0 rounded-2xl overflow-hidden p-2 sm:p-2.5 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 group-hover:border-primary/30 transition-colors">
                        <Image src={exp.icon} alt={getLocalized(exp.company_name)} fill sizes="56px" className="object-contain" />
                      </div>
                      <div>
                        <h3 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white group-hover:text-primary transition-colors leading-tight">{getLocalized(exp.title)}</h3>
                        <p className="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 mt-0.5 sm:mt-1">{getLocalized(exp.company_name)}</p>
                      </div>
                    </div>
                  ...
                  <ul className="space-y-4">
                    {(Array.isArray(getLocalized(exp.points)) ? getLocalized(exp.points) : []).map((point: string, idx: number) => (
                      <li key={idx} className="text-sm text-gray-600 dark:text-gray-400 flex gap-3 leading-relaxed">
